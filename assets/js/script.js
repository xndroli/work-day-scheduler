$(document).ready(function () {
    ...
    var init = false;
    var now = moment().format('dddd, MMM Do'); // Day, Month of Year/Day of Month Format

    const now12 = moment().format('h'); // 12 hour clock format
    const now24 = moment().format('H'); // 24 hour clock format
    if (init) {
        now12 = 1;
        now24 = 13;
    }
